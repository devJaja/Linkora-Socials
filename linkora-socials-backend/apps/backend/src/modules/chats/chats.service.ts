...
export class ChatsService {
  ...
  async getChats(userId: string) {
    ...
    const processedChats = await Promise.all(
      chats.map(async (chat) => {
        ...
        if (!arePopulated) {
          console.log(
            '[getChats] Participants not populated, fetching manually for chat:',
            chat._id,
          );
          console.log(
            '[getChats] Participant types:',
  ...
  async sendTip(chatId: string, userId: string, sendTipDto: SendTipDto) {
    ...
    try {
      ...
      const message = await this.messageModel.create({
        chat: chatId,
        sender: userId,
        content: `Sent ${sendTipDto.amount} XLM`,
        type: 'payment',
      ...
      // Update chat's last message
      await this.chatModel.findByIdAndUpdate(chatId, {
        lastMessage: `Sent ${sendTipDto.amount} XLM`,
        lastMessageAt: new Date(),
        lastMessageBy: userId,
      });
  ...
  async sendPrivateTip(chatId: string, userId: string, sendTipDto: SendTipDto) {
      ...
      participants = (
        await Promise.all(
          participants.map(async (p: any) =>
            this.userModel.findById(p._id || p).lean(),
          ),
        )
      ).filter(Boolean);
    }
    ...
    // Transfer XLM on-chain (custodial path — dWallet path handled in wallet.service)
    const signature = await this.stellarService.sendTransaction(
      sender.walletAddress,
      sender.encryptedPrivateKey,
      recipient.walletAddress,
      sendTipDto.amount,
      'Private tip via Linkora',
    );

    // Encrypt the amount via the Linkora privacy service (pre-alpha mock FHE)
    let ciphertextId: string | undefined;
    try {
      const { encryptAmount } = await import('../../services/encrypt.js');
      const { accountAddress } = await encryptAmount(sendTipDto.amount);
      ciphertextId = accountAddress;
    } catch (err) {
      this.logger.warn(`Encrypt service unavailable: ${err.message}`);
    }
