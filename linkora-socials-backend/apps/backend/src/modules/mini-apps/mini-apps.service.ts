...
export class MiniAppsService {
  ...
  private readonly AIRDROP_AMOUNT = 1;
  ...
  async playDice(
    ...
    const won =
      prediction === 'over' ? roll > targetNumber : roll < targetNumber;

    // Calculate multiplier
    const multiplier =
      prediction === 'over' ? 100 / (100 - targetNumber) : 100 / targetNumber;

    let signature: string | null = null;

    if (won) {
      // User wins - send them XLM
      const winAmount = betAmount * multiplier;
      try {
        // In production, this would come from a house wallet
        // For now, we'll use airdrop on testnet
        signature = await this.stellarService.requestAirdrop(
          user.walletAddress,
        );
      } catch (error) {
      ...
      // User loses - deduct XLM (in a real app, you'd transfer to house wallet)
      // For demo purposes, we'll just record the loss
      this.logger.log(`User ${userId} lost ${betAmount} XLM`);
    }
  ...
  async playCoinFlip(
    ...
    if (won) {
      ...
      this.logger.log(`User ${userId} lost ${betAmount} XLM on coin flip`);
    }
  ...
  async playSpin(userId: string, betAmount: number) {
    ...
    if (selectedMultiplier > 0) {
      ...
      this.logger.log(`User ${userId} lost ${betAmount} XLM on spin`);
    }
  ...
  async swapTokens(
    ...
    try {
      ...
      // If not provided, try to calculate using the Stellar swap service
      if (!toAmount || !rate) {
        try {
          const swapCalc = await this.swapService.calculateSwap(
            fromToken,
            toToken,
            fromAmount,
          );
          swapData = {
            ...
            rate: swapCalc.rate,
            priceImpact: Number(swapCalc.priceImpact) || 0,
          };
        } catch (error) {
          this.logger.warn(
            `Swap calculation failed, using provided values: ${error.message}`,
          );
          // If the calculation fails and no values provided, throw error
          if (!toAmount || !rate) {
      ...
      // Save swap history
      const signature =
        'swap_' + Date.now() + '_' + Math.random().toString(36).substring(7);
      ...
      // 1. Execute the swap transaction on-chain using a Stellar path payment
  ...
  async getTokenPrices() {
    try {
      return await this.swapService.getAllTokenPrices();
    } catch (error) {
  ...
  async claimAirdrop(userId: string) {
    ...
    try {
      // Request airdrop from Stellar testnet (Friendbot)
      const signature = await this.stellarService.requestAirdrop(
        user.walletAddress,
      );
