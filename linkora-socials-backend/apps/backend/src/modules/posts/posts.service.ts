...
import {
  TokenHolder,
  TokenHolderDocument,
} from '../../schemas/token-holder.schema';
...
export class PostsService {
  constructor(
    ...
    @InjectModel(PostToken.name)
    private postTokenModel: Model<PostTokenDocument>,
    @InjectModel(TokenHolder.name)
    private tokenHolderModel: Model<TokenHolderDocument>,
  ...
  async createPost(userId: string, createPostDto: CreatePostDto) {
    ...
    if (
      createPostDto.isTokenized &&
      createPostDto.tokenSupply &&
      createPostDto.tokenPrice
    ) {
      // Generate a mock token asset code (in production, create an actual Stellar asset)
  ...
  async getFeed(userId: string, page: number = 1, limit: number = 20) {
    ...
    const postsWithLikeStatus = await Promise.all(
      posts.map(async (post: any) => {
        ...
        const liked = await this.likeModel
          .findOne({
  ...
  async getUserPosts(
    ...
    console.log(
      '[getUserPosts] Called with username:',
      username,
      'userId:',
      userId,
    );
    ...
    console.log(
      '[getUserPosts] Found user:',
      user ? { _id: user._id, username: user.username } : 'NOT FOUND',
    );
    ...
    console.log(
      '[getUserPosts] Found posts count after pagination:',
      posts.length,
    );
    console.log(
      '[getUserPosts] First post:',
      posts[0]
        ? { _id: posts[0]._id, content: posts[0].content?.substring(0, 50) }
        : 'NONE',
    );
    ...
    console.log(
      '[getUserPosts] Returning posts count:',
  ...
  async getComments(
    ...
    const commentsWithLikes = await Promise.all(
      comments.map(async (comment) => {
        // Convert userId string to ObjectId for proper comparison
        const liked = await this.commentLikeModel
          .findOne({
            comment: comment._id,
            user: new Types.ObjectId(userId),
          })
  ...
  async buyPostToken(postId: string, userId: string, amount: number) {
    console.log(
      `[BuyToken] Starting purchase: postId=${postId}, userId=${userId}, amount=${amount}`,
    );
    ...
    console.log(
      `[BuyToken] Post found: isTokenized=${post.isTokenized}, tokenSupply=${post.tokenSupply}, tokenPrice=${post.tokenPrice}`,
    );
    ...
    if (!postToken) {
      if (!post.tokenSupply || post.tokenSupply <= 0) {
        ...
        throw new BadRequestException(
          'Post token supply is not configured. Please contact the post author.',
        );
      }
      ...
      if (!post.tokenPrice || post.tokenPrice <= 0) {
        ...
        throw new BadRequestException(
          'Post token price is not configured. Please contact the post author.',
        );
      }
    ...
    console.log(
      `[BuyToken] Available tokens: ${availableTokens}, Requested: ${amount}`,
    );

    if (amount > availableTokens) {
      throw new BadRequestException(
        `Not enough tokens available. Only ${availableTokens} tokens remaining.`,
      );
    }

    const totalCost = amount * postToken.currentPrice;
    console.log(`[BuyToken] Total cost: ${totalCost} XLM`);
    ...
    console.log(
      `[BuyToken] Initiating XLM transfer from ${buyer.walletAddress} to ${seller.walletAddress}`,
    );
    ...
    postToken.currentPrice =
      postToken.initialPrice *
      (1 + (postToken.soldTokens / postToken.totalSupply) * 0.1);
  ...
  async tipPost(
    ...
    await this.notificationsService.createNotification({
      ...
      sender: userId,
      type: 'tip',
      message: `${tipper.username} tipped you ${amount} XLM on your post`,
      post: postId,
      amount,
    });
  ...
  async getUserPortfolio(userId: string) {
    ...
    const portfolioItems = await Promise.all(
      holdings.map(async (holding: any) => {
        const postToken = await this.postTokenModel.findOne({
          post: holding.post._id,
        });
        ...
        return {
          ...
          profitLossPercentage:
            ((currentValue - holding.totalInvested) / holding.totalInvested) *
            100,
        };
      }),
    );
  ...
  async getUserLikes(
    ...
    const posts = await Promise.all(
      likes.map(async (like: any) => {
        ...
        // Convert userId string to ObjectId for proper comparison
        const isLiked = await this.likeModel
          .findOne({
            post: post._id,
