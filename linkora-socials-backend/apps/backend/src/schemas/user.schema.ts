...
export class User {
  ...
  @Prop({ default: 0 })
  portfolioValue: number; // Total value of owned post tokens in XLM

  @Prop({ default: 0 })
  totalInvested: number; // Total XLM invested in post tokens

  @Prop()
  lastAirdropClaim: Date; // Last time user claimed airdrop

  @Prop({ default: 0 })
  totalAirdropClaimed: number; // Total XLM claimed from airdrops
  @Prop({ default: 0 })
