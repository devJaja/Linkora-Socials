 ...
 * linkora-privacy on-chain program to enable confidential tips in chat.
 *
...
export const LINKORA_PRIVACY_PROGRAM_ID = new PublicKey(
  process.env.LINKORA_PRIVACY_PROGRAM_ID ?? '11111111111111111111111111111112',
);
 ...
 *
 * The ciphertext is authorised for the linkora-privacy program only —
...
export async function encryptAmount(
  ...
  const { ciphertextIdentifiers } = await client.createInput({
    ...
    authorized: Buffer.from(LINKORA_PRIVACY_PROGRAM_ID.toBytes()),
 ...
 *
 * Calls `execute_private_tip` on the linkora-privacy program, which CPI-calls
...
export async function executePrivateTip(
  ...
  const ix = {
    programId: LINKORA_PRIVACY_PROGRAM_ID,
    keys: [
      ...
      { pubkey: LINKORA_PRIVACY_PROGRAM_ID, isSigner: false, isWritable: false },
